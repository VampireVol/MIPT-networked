...
void send_new_entity(ENetPeer *peer, const Entity &ent, uint32_t t)
{
  ENetPacket *packet = enet_packet_create(nullptr, sizeof(uint8_t) + sizeof(Entity) + sizeof(uint32_t),
                                                   ENET_PACKET_FLAG_RELIABLE);
  uint8_t *ptr = packet->data;
  *ptr = E_SERVER_TO_CLIENT_NEW_ENTITY; ptr += sizeof(uint8_t);
  memcpy(ptr, &ent, sizeof(Entity)); ptr += sizeof(Entity);
  memcpy(ptr, &t, sizeof(uint32_t)); ptr += sizeof(uint32_t);

  enet_peer_send(peer, 0, packet);
}

void send_set_controlled_entity(ENetPeer *peer, uint16_t eid, uint32_t t)
{
...
void send_snapshot(ENetPeer *peer, uint16_t eid, float x, float y, float ori, uint32_t t)
{
  ENetPacket *packet = enet_packet_create(nullptr, sizeof(uint8_t) + sizeof(uint16_t) +
...
void deserialize_new_entity(ENetPacket *packet, Entity &ent, uint32_t &t)
{
  uint8_t *ptr = packet->data; ptr += sizeof(uint8_t);
  ent = *(Entity*)(ptr); ptr += sizeof(Entity);
  t = *(uint32_t*)(ptr); ptr += sizeof(uint32_t);
}

void deserialize_set_controlled_entity(ENetPacket *packet, uint16_t &eid, uint32_t& t)
{
  uint8_t *ptr = packet->data; ptr += sizeof(uint8_t);
...
void deserialize_snapshot(ENetPacket *packet, uint16_t &eid, float &x, float &y, float &ori, uint32_t &t)
{
  uint8_t *ptr = packet->data; ptr += sizeof(uint8_t);
  eid = *(uint16_t*)(ptr); ptr += sizeof(uint16_t);
